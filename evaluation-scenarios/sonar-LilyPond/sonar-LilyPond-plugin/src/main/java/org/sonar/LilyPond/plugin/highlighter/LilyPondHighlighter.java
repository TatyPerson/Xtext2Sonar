package org.sonar.LilyPond.plugin.highlighter;
...
public class LilyPondHighlighter {

  private Lexer lexer;
  private Charset charset;

  public LilyPondHighlighter(LilyPondConfiguration conf) {
    this.lexer = LilyPondLexer.create(conf);
  ...
  private void highlightNonComments(Highlightable.HighlightingBuilder highlighting, List<Token> tokens, SourceFileOffsets offsets) {
    for (Token token : tokens) {
      if (LilyPondTokenType.STRING.equals(token.getType())) {
  ...
  private boolean isConstant(TokenType type) {
    return LilyPondTokenType.NUMBER.equals(type)
      || LilyPondTokenType.CHARACTER.equals(type);
  }

  private boolean isKeyword(TokenType type) {
    for (TokenType keywordType : LilyPondKeyword.values()) {
