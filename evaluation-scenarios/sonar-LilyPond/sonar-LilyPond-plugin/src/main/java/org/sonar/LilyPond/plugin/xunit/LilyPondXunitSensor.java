 ...
 * {@inheritDoc}
 */
public class LilyPondXunitSensor extends LilyPondReportSensor {
  public static final String REPORT_PATH_KEY = "sonar.lilypond.xunit.reportPath";
  public static final String XSLT_URL_KEY = "sonar.lilypond.xunit.xsltURL";
  public static final String PROVIDE_DETAILS_KEY = "sonar.lilypond.xunit.provideDetails";
   ...
   * {@inheritDoc}
   */
  public LilyPondXunitSensor(Settings conf, FileSystem fs, ProjectReactor reactor) {
  ...
  public boolean shouldExecuteOnProject(Project project) {
    ...
    return !project.getFileSystem().mainFiles(LilyPondLanguage.KEY).isEmpty();
  }
      ...
      if (!reports.isEmpty()) {
        ...
        for (File report : reports) {
          ...
          try {
            ...
            LilyPondUtils.LOG.warn("The report '{}' seems to be empty, ignoring.", report);
          }
        }
        List<TestCase> testcases = parserHandler.getTestCases();

        LilyPondUtils.LOG.info("Parsing 'xUnit' format");
        boolean providedetails = conf.getBoolean(PROVIDE_DETAILS_KEY);
        if (providedetails) {
        ...
        LilyPondUtils.LOG.debug("No reports found, nothing to process");
      }
    } catch (Exception e) {
  ...
  {
    LilyPondUtils.LOG.info("Processing in 'simple mode' i.e. with provideDetails=false.");
    ...
    if (testsCount > 0) {
      ...
      LilyPondUtils.LOG.debug("The reports contain no testcases");
    }
  }
  ...
  private void detailledMode(final Project project, final SensorContext context, List<TestCase> testcases)
    ...
    javax.xml.transform.TransformerException
  {
    LilyPondUtils.LOG.info("Processing in 'detailled mode' i.e. with provideDetails=true");

    String sonarTests = conf.getString("sonar.tests");
    if (sonarTests == null || "".equals(sonarTests)){
      LilyPondUtils.LOG.error("The property 'sonar.tests' is unset. Please set it to proceed");
      return;
    }
    ...
    LilyPondUtils.LOG.info("Summary: testcases processed = {}, skipped = {}", tcTotal, tcSkipped);
    if (tcSkipped > 0){
      LilyPondUtils.LOG.warn("Some testcases had to be skipped, check the relevant parts of your setup "
  ...
  private Collection<TestResource> lookupResources(Project project, SensorContext context, List<TestCase> testcases) {
    ...
    for (TestCase tc : testcases) {
      ...
      LilyPondUtils.LOG.debug("Trying the resource for the testcase '{}' ...", tc.getFullname());
      org.sonar.api.resources.File sonarResource = lookupResource(project, context, tc);
      if (sonarResource != null) {
        LilyPondUtils.LOG.debug("... found! The resource is '{}'", sonarResource);
        ...
        tcSkipped++;
        LilyPondUtils.LOG.warn("... no resource found, the testcase '{}' has to be skipped",
                          tc.getFullname());
      }
    }
      ...
      throws java.io.IOException, javax.xml.transform.TransformerException
  {
    ...
    if (xsltURL != null && report.length() > 0) {
      LilyPondUtils.LOG.debug("Transforming the report using xslt '{}'", xsltURL);
      InputStream inputStream = this.getClass().getResourceAsStream("/xsl/" + xsltURL);
      if (inputStream == null) {
        LilyPondUtils.LOG.debug("Transforming: try to access external XSLT via URL");
      ...
      LilyPondUtils.LOG.debug("Transformation skipped: no xslt given");
    }
  ...
  private org.sonar.api.resources.File lookupResource(Project project, SensorContext context, TestCase tc) {
    ...
    if (filepath != null){
      LilyPondUtils.LOG.debug("Performing the 'filename'-based lookup using the value '{}'", filepath);
    ...
    if (classname != null){
      ...
      LilyPondUtils.LOG.debug("Performing AST-based lookup, determined file path: '{}'", filepath);
      sonarResource = lookupInSonar(filepath, context, project);
    } else {
      LilyPondUtils.LOG.debug("Skipping the AST-based lookup: no classname provided");
    }
  ...
  void buildLookupTables() {
    ...
    LilyPondConfiguration cxxConf = new LilyPondConfiguration(fs.encoding());
    cxxConf.setBaseDir(fs.baseDir().getAbsolutePath());
    for (File file : files) {
      @SuppressWarnings("unchecked")
      SourceFile source = LilyPondAstScanner.scanSingleFileConfig(file, cxxConf);
