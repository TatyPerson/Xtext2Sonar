...
@Rule(
		key = "ComposerIsDefinedCheck",
		name = "The music sheet composer must be defined.",
		description = "Verifies that the music sheet composer have been defined.",
		priority = Priority.MINOR)
		@ActivatedByDefault
		@SqaleSubCharacteristic(RulesDefinition.SubCharacteristics.READABILITY)
		@SqaleConstantRemediation("1min")
public class ComposerIsDefinedCheck extends SquidCheck<Grammar> {
	
	private static final Logger LOG = LoggerFactory.getLogger("ComposerIsDefinedCheck");
	private static final String COMPOSER_KEYWORD = "composer";
	...
	public void leaveFile(AstNode node) {
		if(!attributes.contains(COMPOSER_KEYWORD)) {
			getContext().createLineViolation(this, " The music sheet composer must have been defined.", headerLine);
		}
	}
}
