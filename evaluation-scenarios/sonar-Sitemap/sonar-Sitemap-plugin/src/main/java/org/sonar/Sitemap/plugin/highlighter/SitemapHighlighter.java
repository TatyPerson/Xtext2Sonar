package org.sonar.Sitemap.plugin.highlighter;
...
public class SitemapHighlighter {

  private Lexer lexer;
  private Charset charset;

  public SitemapHighlighter(SitemapConfiguration conf) {
    this.lexer = SitemapLexer.create(conf);
  ...
  private void highlightNonComments(Highlightable.HighlightingBuilder highlighting, List<Token> tokens, SourceFileOffsets offsets) {
    for (Token token : tokens) {
      if (SitemapTokenType.STRING.equals(token.getType())) {
  ...
  private boolean isConstant(TokenType type) {
    return SitemapTokenType.NUMBER.equals(type)
      || SitemapTokenType.CHARACTER.equals(type);
  }

  private boolean isKeyword(TokenType type) {
    for (TokenType keywordType : SitemapKeyword.values()) {
