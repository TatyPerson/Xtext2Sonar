package org.sonar.Sitemap.plugin.coverage;
 ...
 * {@inheritDoc}
 */
public class SitemapCoverageSensor extends SitemapReportSensor {
  private enum CoverageType {
    UT_COVERAGE, IT_COVERAGE, OVERALL_COVERAGE
  }

  public static final String REPORT_PATH_KEY = "sonar.sitemap.coverage.reportPath";
  public static final String IT_REPORT_PATH_KEY = "sonar.sitemap.coverage.itReportPath";
  public static final String OVERALL_REPORT_PATH_KEY = "sonar.sitemap.coverage.overallReportPath";
  ...
  public static final String FORCE_ZERO_COVERAGE_KEY = "sonar.sitemap.coverage.forceZeroCoverage";
   ...
   * {@inheritDoc}
   */
  public SitemapCoverageSensor(Settings settings, FileSystem fs, ProjectReactor reactor) {
  ...
  public void analyse(Project project, SensorContext context) {
    ...
    SitemapUtils.LOG.debug("Parsing coverage reports");
    ...
    SitemapUtils.LOG.debug("Parsing integration test coverage reports");
    ...
    SitemapUtils.LOG.debug("Parsing overall test coverage reports");
    ...
    if (isForceZeroCoverageActivated()) {
      SitemapUtils.LOG.debug("Zeroing coverage information for untouched files");
      ...
      for (CoverageParser parser : parsers) {
        try {
          ...
          if (!measuresForReport.isEmpty()) {
            ...
            SitemapUtils.LOG.info("Added report '{}' (parsed by: {}) to the coverage data", report, parser);
            break;
          }
        } catch (XMLStreamException e) {
          SitemapUtils.LOG.trace("Report {} cannot be parsed by {}", report, parser);
        }
      }

      if (!parsed) {
        SitemapUtils.LOG.error("Report {} cannot be parsed", report);
      }
    }
  ...
  private void saveMeasures(Project project,
    ...
    for (Map.Entry<String, CoverageMeasuresBuilder> entry : coverageMeasures.entrySet()) {
      ...
      if (fileExist(context, cxxfile)) {
        SitemapUtils.LOG.debug("Saving coverage measures for file '{}'", filePath);
        ...
        SitemapUtils.LOG.warn("Cannot find the file '{}', ignoring coverage measures", filePath);
      }
    }
  }
  ...
  private void zeroMeasuresWithoutReports(Project project,
    ...
    for (File file : fs.files(fs.predicates().hasLanguage(SitemapLanguage.KEY))) {
      org.sonar.api.resources.File resource = org.sonar.api.resources.File.fromIOFile(file, project);
      if (fileExist(context, resource)) {
        String filePath = SitemapUtils.normalizePath(file.getAbsolutePath());
  ...
  private void saveZeroValueForResource(org.sonar.api.resources.File resource,
    ...
    if (ncloc != null && stmts != null
      ...
      SitemapUtils.LOG.debug("Zeroing {}coverage measures for file '{}'", coverageKind, filePath);
