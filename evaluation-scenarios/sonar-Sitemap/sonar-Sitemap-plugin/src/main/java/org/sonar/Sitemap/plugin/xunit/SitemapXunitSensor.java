...
import org.sonar.Sitemap.api.SitemapAstScanner;
import org.sonar.Sitemap.SitemapConfiguration;
import org.sonar.Sitemap.plugin.SitemapLanguage;
import org.sonar.Sitemap.plugin.SitemapPlugin;
import org.sonar.Sitemap.plugin.utils.SitemapReportSensor;
import org.sonar.Sitemap.plugin.utils.SitemapUtils;
import org.sonar.Sitemap.plugin.utils.EmptyReportException;
 ...
 * {@inheritDoc}
 */
public class SitemapXunitSensor extends SitemapReportSensor {
  public static final String REPORT_PATH_KEY = "sonar.sitemap.xunit.reportPath";
  public static final String XSLT_URL_KEY = "sonar.sitemap.xunit.xsltURL";
  public static final String PROVIDE_DETAILS_KEY = "sonar.sitemap.xunit.provideDetails";
   ...
   * {@inheritDoc}
   */
  public SitemapXunitSensor(Settings conf, FileSystem fs, ProjectReactor reactor) {
  ...
  public boolean shouldExecuteOnProject(Project project) {
    ...
    return !project.getFileSystem().mainFiles(SitemapLanguage.KEY).isEmpty();
  }
    ...
    try{
      ...
      if (!reports.isEmpty()) {
        ...
        for (File report : reports) {
          SitemapUtils.LOG.info("Processing report '{}'", report);
          try {
            parser.parse(transformReport(report));
          } catch (EmptyReportException e) {
            SitemapUtils.LOG.warn("The report '{}' seems to be empty, ignoring.", report);
          }
        }
        List<TestCase> testcases = parserHandler.getTestCases();

        SitemapUtils.LOG.info("Parsing 'xUnit' format");
        boolean providedetails = conf.getBoolean(PROVIDE_DETAILS_KEY);
        if (providedetails) {
        ...
        SitemapUtils.LOG.debug("No reports found, nothing to process");
      }
    } catch (Exception e) {
  ...
  {
    SitemapUtils.LOG.info("Processing in 'simple mode' i.e. with provideDetails=false.");
    ...
    if (testsCount > 0) {
      ...
      SitemapUtils.LOG.debug("The reports contain no testcases");
    }
  }
  ...
  private void detailledMode(final Project project, final SensorContext context, List<TestCase> testcases)
    ...
    javax.xml.transform.TransformerException
  {
    SitemapUtils.LOG.info("Processing in 'detailled mode' i.e. with provideDetails=true");

    String sonarTests = conf.getString("sonar.tests");
    if (sonarTests == null || "".equals(sonarTests)){
      SitemapUtils.LOG.error("The property 'sonar.tests' is unset. Please set it to proceed");
      return;
    }
    ...
    SitemapUtils.LOG.info("Summary: testcases processed = {}, skipped = {}", tcTotal, tcSkipped);
    if (tcSkipped > 0){
      SitemapUtils.LOG.warn("Some testcases had to be skipped, check the relevant parts of your setup "
  ...
  private Collection<TestResource> lookupResources(Project project, SensorContext context, List<TestCase> testcases) {
    ...
    for (TestCase tc : testcases) {
      ...
      SitemapUtils.LOG.debug("Trying the resource for the testcase '{}' ...", tc.getFullname());
      org.sonar.api.resources.File sonarResource = lookupResource(project, context, tc);
      if (sonarResource != null) {
        SitemapUtils.LOG.debug("... found! The resource is '{}'", sonarResource);
        ...
        tcSkipped++;
        SitemapUtils.LOG.warn("... no resource found, the testcase '{}' has to be skipped",
                          tc.getFullname());
      }
    }
      ...
      throws java.io.IOException, javax.xml.transform.TransformerException
  {
    ...
    if (xsltURL != null && report.length() > 0) {
      SitemapUtils.LOG.debug("Transforming the report using xslt '{}'", xsltURL);
      InputStream inputStream = this.getClass().getResourceAsStream("/xsl/" + xsltURL);
      if (inputStream == null) {
        SitemapUtils.LOG.debug("Transforming: try to access external XSLT via URL");
      ...
      SitemapUtils.LOG.debug("Transformation skipped: no xslt given");
    }
  ...
  private org.sonar.api.resources.File lookupResource(Project project, SensorContext context, TestCase tc) {
    ...
    if (filepath != null){
      SitemapUtils.LOG.debug("Performing the 'filename'-based lookup using the value '{}'", filepath);
    ...
    if (classname != null){
      ...
      SitemapUtils.LOG.debug("Performing AST-based lookup, determined file path: '{}'", filepath);
      sonarResource = lookupInSonar(filepath, context, project);
    } else {
      SitemapUtils.LOG.debug("Skipping the AST-based lookup: no classname provided");
    }
  ...
  void buildLookupTables() {
    ...
    Iterable<File> files = fs.files(predicates.and(
        ...
        predicates.hasLanguage(SitemapLanguage.KEY)));

    SitemapConfiguration cxxConf = new SitemapConfiguration(fs.encoding());
    cxxConf.setBaseDir(fs.baseDir().getAbsolutePath());
    for (File file : files) {
      @SuppressWarnings("unchecked")
      SourceFile source = SitemapAstScanner.scanSingleFileConfig(file, cxxConf);
