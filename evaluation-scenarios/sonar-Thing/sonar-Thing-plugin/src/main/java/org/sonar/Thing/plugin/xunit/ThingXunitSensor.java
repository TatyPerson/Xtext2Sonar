...
import org.sonar.Thing.plugin.utils.EmptyReportException;
 ...
 * {@inheritDoc}
 */
public class ThingXunitSensor extends ThingReportSensor {
  public static final String REPORT_PATH_KEY = "sonar.thing.xunit.reportPath";
  public static final String XSLT_URL_KEY = "sonar.thing.xunit.xsltURL";
  public static final String PROVIDE_DETAILS_KEY = "sonar.thing.xunit.provideDetails";
   ...
   * {@inheritDoc}
   */
  public ThingXunitSensor(Settings conf, FileSystem fs, ProjectReactor reactor) {
  ...
  public boolean shouldExecuteOnProject(Project project) {
    ...
    return !project.getFileSystem().mainFiles(ThingLanguage.KEY).isEmpty();
  }
    ...
    try{
      ...
      if (!reports.isEmpty()) {
        ...
        for (File report : reports) {
          ThingUtils.LOG.info("Processing report '{}'", report);
          try {
            parser.parse(transformReport(report));
          } catch (EmptyReportException e) {
            ThingUtils.LOG.warn("The report '{}' seems to be empty, ignoring.", report);
          }
        }
        List<TestCase> testcases = parserHandler.getTestCases();

        ThingUtils.LOG.info("Parsing 'xUnit' format");
        boolean providedetails = conf.getBoolean(PROVIDE_DETAILS_KEY);
        if (providedetails) {
        ...
        ThingUtils.LOG.debug("No reports found, nothing to process");
      }
    } catch (Exception e) {
  ...
  {
    ThingUtils.LOG.info("Processing in 'simple mode' i.e. with provideDetails=false.");
    ...
    if (testsCount > 0) {
      ...
      ThingUtils.LOG.debug("The reports contain no testcases");
    }
  }
  ...
  private void detailledMode(final Project project, final SensorContext context, List<TestCase> testcases)
    ...
    javax.xml.transform.TransformerException
  {
    ThingUtils.LOG.info("Processing in 'detailled mode' i.e. with provideDetails=true");

    String sonarTests = conf.getString("sonar.tests");
    if (sonarTests == null || "".equals(sonarTests)){
      ThingUtils.LOG.error("The property 'sonar.tests' is unset. Please set it to proceed");
      return;
    }
    ...
    ThingUtils.LOG.info("Summary: testcases processed = {}, skipped = {}", tcTotal, tcSkipped);
    if (tcSkipped > 0){
      ThingUtils.LOG.warn("Some testcases had to be skipped, check the relevant parts of your setup "
  ...
  private Collection<TestResource> lookupResources(Project project, SensorContext context, List<TestCase> testcases) {
    ...
    for (TestCase tc : testcases) {
      ...
      ThingUtils.LOG.debug("Trying the resource for the testcase '{}' ...", tc.getFullname());
      org.sonar.api.resources.File sonarResource = lookupResource(project, context, tc);
      if (sonarResource != null) {
        ThingUtils.LOG.debug("... found! The resource is '{}'", sonarResource);
        ...
        tcSkipped++;
        ThingUtils.LOG.warn("... no resource found, the testcase '{}' has to be skipped",
                          tc.getFullname());
      }
    }
      ...
      throws java.io.IOException, javax.xml.transform.TransformerException
  {
    ...
    if (xsltURL != null && report.length() > 0) {
      ThingUtils.LOG.debug("Transforming the report using xslt '{}'", xsltURL);
      InputStream inputStream = this.getClass().getResourceAsStream("/xsl/" + xsltURL);
      if (inputStream == null) {
        ThingUtils.LOG.debug("Transforming: try to access external XSLT via URL");
      ...
      ThingUtils.LOG.debug("Transformation skipped: no xslt given");
    }
  ...
  private org.sonar.api.resources.File lookupResource(Project project, SensorContext context, TestCase tc) {
    ...
    if (filepath != null){
      ThingUtils.LOG.debug("Performing the 'filename'-based lookup using the value '{}'", filepath);
    ...
    if (classname != null){
      ...
      ThingUtils.LOG.debug("Performing AST-based lookup, determined file path: '{}'", filepath);
      sonarResource = lookupInSonar(filepath, context, project);
    } else {
      ThingUtils.LOG.debug("Skipping the AST-based lookup: no classname provided");
    }
  ...
  void buildLookupTables() {
    ...
    ThingConfiguration cxxConf = new ThingConfiguration(fs.encoding());
    cxxConf.setBaseDir(fs.baseDir().getAbsolutePath());
    for (File file : files) {
      @SuppressWarnings("unchecked")
      SourceFile source = ThingAstScanner.scanSingleFileConfig(file, cxxConf);
