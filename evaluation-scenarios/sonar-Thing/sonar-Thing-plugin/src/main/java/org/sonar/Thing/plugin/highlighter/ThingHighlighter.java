package org.sonar.Thing.plugin.highlighter;
...
public class ThingHighlighter {

  private Lexer lexer;
  private Charset charset;

  public ThingHighlighter(ThingConfiguration conf) {
    this.lexer = ThingLexer.create(conf);
  ...
  private void highlightNonComments(Highlightable.HighlightingBuilder highlighting, List<Token> tokens, SourceFileOffsets offsets) {
    for (Token token : tokens) {
      if (ThingTokenType.STRING.equals(token.getType())) {
  ...
  private boolean isConstant(TokenType type) {
    return ThingTokenType.NUMBER.equals(type)
      || ThingTokenType.CHARACTER.equals(type);
  }

  private boolean isKeyword(TokenType type) {
    for (TokenType keywordType : ThingKeyword.values()) {
