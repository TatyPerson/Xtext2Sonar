package org.sonar.Thing.plugin.coverage;
 ...
 * {@inheritDoc}
 */
public class ThingCoverageSensor extends ThingReportSensor {
  private enum CoverageType {
    UT_COVERAGE, IT_COVERAGE, OVERALL_COVERAGE
  }

  public static final String REPORT_PATH_KEY = "sonar.thing.coverage.reportPath";
  public static final String IT_REPORT_PATH_KEY = "sonar.thing.coverage.itReportPath";
  public static final String OVERALL_REPORT_PATH_KEY = "sonar.thing.coverage.overallReportPath";
  ...
  public static final String FORCE_ZERO_COVERAGE_KEY = "sonar.thing.coverage.forceZeroCoverage";
   ...
   * {@inheritDoc}
   */
  public ThingCoverageSensor(Settings settings, FileSystem fs, ProjectReactor reactor) {
  ...
  public void analyse(Project project, SensorContext context) {
    ...
    ThingUtils.LOG.debug("Parsing coverage reports");
    ...
    ThingUtils.LOG.debug("Parsing integration test coverage reports");
    ...
    ThingUtils.LOG.debug("Parsing overall test coverage reports");
    ...
    if (isForceZeroCoverageActivated()) {
      ThingUtils.LOG.debug("Zeroing coverage information for untouched files");
      ...
      for (CoverageParser parser : parsers) {
        try {
          ...
          if (!measuresForReport.isEmpty()) {
            ...
            ThingUtils.LOG.info("Added report '{}' (parsed by: {}) to the coverage data", report, parser);
            break;
          }
        } catch (XMLStreamException e) {
          ThingUtils.LOG.trace("Report {} cannot be parsed by {}", report, parser);
        }
      }

      if (!parsed) {
        ThingUtils.LOG.error("Report {} cannot be parsed", report);
      }
    }
  ...
  private void saveMeasures(Project project,
    ...
    for (Map.Entry<String, CoverageMeasuresBuilder> entry : coverageMeasures.entrySet()) {
      ...
      if (fileExist(context, cxxfile)) {
        ThingUtils.LOG.debug("Saving coverage measures for file '{}'", filePath);
        ...
        ThingUtils.LOG.warn("Cannot find the file '{}', ignoring coverage measures", filePath);
      }
    }
  }
  ...
  private void zeroMeasuresWithoutReports(Project project,
    ...
    for (File file : fs.files(fs.predicates().hasLanguage(ThingLanguage.KEY))) {
      org.sonar.api.resources.File resource = org.sonar.api.resources.File.fromIOFile(file, project);
      if (fileExist(context, resource)) {
        String filePath = ThingUtils.normalizePath(file.getAbsolutePath());
  ...
  private void saveZeroValueForResource(org.sonar.api.resources.File resource,
    ...
    if (ncloc != null && stmts != null
      ...
      ThingUtils.LOG.debug("Zeroing {}coverage measures for file '{}'", coverageKind, filePath);
