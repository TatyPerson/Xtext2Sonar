package org.sonar.VaryGrammar.plugin.highlighter;
...
public class VaryGrammarHighlighter {

  private Lexer lexer;
  private Charset charset;

  public VaryGrammarHighlighter(VaryGrammarConfiguration conf) {
    this.lexer = VaryGrammarLexer.create(conf);
  ...
  private void highlightNonComments(Highlightable.HighlightingBuilder highlighting, List<Token> tokens, SourceFileOffsets offsets) {
    for (Token token : tokens) {
      if (VaryGrammarTokenType.STRING.equals(token.getType())) {
  ...
  private boolean isConstant(TokenType type) {
    return VaryGrammarTokenType.NUMBER.equals(type)
      || VaryGrammarTokenType.CHARACTER.equals(type);
  }

  private boolean isKeyword(TokenType type) {
    for (TokenType keywordType : VaryGrammarKeyword.values()) {
