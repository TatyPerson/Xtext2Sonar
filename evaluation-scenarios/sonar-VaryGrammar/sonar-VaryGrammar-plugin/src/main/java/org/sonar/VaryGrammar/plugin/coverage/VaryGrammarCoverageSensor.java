package org.sonar.VaryGrammar.plugin.coverage;
 ...
 * {@inheritDoc}
 */
public class VaryGrammarCoverageSensor extends VaryGrammarReportSensor {
  private enum CoverageType {
    UT_COVERAGE, IT_COVERAGE, OVERALL_COVERAGE
  }

  public static final String REPORT_PATH_KEY = "sonar.varygrammar.coverage.reportPath";
  public static final String IT_REPORT_PATH_KEY = "sonar.varygrammar.coverage.itReportPath";
  public static final String OVERALL_REPORT_PATH_KEY = "sonar.varygrammar.coverage.overallReportPath";
  ...
  public static final String FORCE_ZERO_COVERAGE_KEY = "sonar.varygrammar.coverage.forceZeroCoverage";
   ...
   * {@inheritDoc}
   */
  public VaryGrammarCoverageSensor(Settings settings, FileSystem fs, ProjectReactor reactor) {
  ...
  public void analyse(Project project, SensorContext context) {
    ...
    VaryGrammarUtils.LOG.debug("Parsing coverage reports");
    ...
    VaryGrammarUtils.LOG.debug("Parsing integration test coverage reports");
    ...
    VaryGrammarUtils.LOG.debug("Parsing overall test coverage reports");
    ...
    if (isForceZeroCoverageActivated()) {
      VaryGrammarUtils.LOG.debug("Zeroing coverage information for untouched files");
      ...
      for (CoverageParser parser : parsers) {
        try {
          ...
          if (!measuresForReport.isEmpty()) {
            ...
            VaryGrammarUtils.LOG.info("Added report '{}' (parsed by: {}) to the coverage data", report, parser);
            break;
          }
        } catch (XMLStreamException e) {
          VaryGrammarUtils.LOG.trace("Report {} cannot be parsed by {}", report, parser);
        }
      }

      if (!parsed) {
        VaryGrammarUtils.LOG.error("Report {} cannot be parsed", report);
      }
    }
  ...
  private void saveMeasures(Project project,
    ...
    for (Map.Entry<String, CoverageMeasuresBuilder> entry : coverageMeasures.entrySet()) {
      ...
      if (fileExist(context, cxxfile)) {
        VaryGrammarUtils.LOG.debug("Saving coverage measures for file '{}'", filePath);
        ...
        VaryGrammarUtils.LOG.warn("Cannot find the file '{}', ignoring coverage measures", filePath);
      }
    }
  }
  ...
  private void zeroMeasuresWithoutReports(Project project,
    ...
    for (File file : fs.files(fs.predicates().hasLanguage(VaryGrammarLanguage.KEY))) {
      org.sonar.api.resources.File resource = org.sonar.api.resources.File.fromIOFile(file, project);
      if (fileExist(context, resource)) {
        String filePath = VaryGrammarUtils.normalizePath(file.getAbsolutePath());
  ...
  private void saveZeroValueForResource(org.sonar.api.resources.File resource,
    ...
    if (ncloc != null && stmts != null
      ...
      VaryGrammarUtils.LOG.debug("Zeroing {}coverage measures for file '{}'", coverageKind, filePath);
