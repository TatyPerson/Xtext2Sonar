 ...
 * in SonarQube
 */
public abstract class ItemsReportSensor implements Sensor {
   ...
   * @param reactor
   */
  protected ItemsReportSensor(Settings conf, FileSystem fs, ProjectReactor reactor) {
   ...
   * @param reactor
   */
  protected ItemsReportSensor(ResourcePerspectives perspectives, Settings conf, FileSystem fs, ProjectReactor reactor, Metric metric) {
    this.conf = conf;
    this.fs = fs;
    this.metric = metric;
  ...
  public boolean shouldExecuteOnProject(Project project) {
    return fs.hasFiles(fs.predicates().hasLanguage(ItemsLanguage.KEY));
  }
  ...
  public void analyse(Project project, SensorContext context) {
    if (!ItemsUtils.isReactorProject(project)) {
      try {
      List<File> reports = getReports(conf, reactor.getRoot().getBaseDir().getCanonicalPath(),
        ...
        try {
          ...
          ItemsUtils.LOG.info("{} processed = {}", metric == null ? "Issues" : metric.getName(),
  ...
  public static List<File> getReports(Settings conf,
    ...
    ItemsUtils.LOG.debug("Using pattern '{}' to find reports", reportPath);
    ...
    for (String relPath : relPaths) {
      String path = ItemsUtils.normalizePath(new File(baseDirPath, relPath).getAbsolutePath());      
  ...
  private boolean saveViolation(Project project, SensorContext context, String ruleRepoKey,
    ...
    if ((filename != null) && (filename.length() > 0)) {
      ...
      String normalPath = ItemsUtils.normalizePathFull(filename, root);
      if (normalPath != null && !notFoundFiles.contains(normalPath)) {
        ...
        if (context.getResource(file) != null) {
          ...
          ItemsUtils.LOG.warn("Cannot find the file '{}', skipping violations", normalPath);
  ...
  private boolean contextSaveViolation(Resource resource, int lineNr, RuleKey rule, String msg) {
    ...
    if (issuable != null) {
      ...
      try{
        ...
        ItemsUtils.LOG.error("Could not add the issue, details: '{}'", me.toString());
      }
      if (result)
        violationsCount++;
    }
  ...
  private int getLineAsInt(String line) {
    ...
    if (line != null) {
      try {
        ...
        ItemsUtils.LOG.warn("Skipping invalid line number: {}", line);
        lineNr = -1;
      }
    }
    return lineNr;
