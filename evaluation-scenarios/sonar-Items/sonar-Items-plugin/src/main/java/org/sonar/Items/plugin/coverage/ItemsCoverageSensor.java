package org.sonar.Items.plugin.coverage;
 ...
 * {@inheritDoc}
 */
public class ItemsCoverageSensor extends ItemsReportSensor {
  private enum CoverageType {
    UT_COVERAGE, IT_COVERAGE, OVERALL_COVERAGE
  }

  public static final String REPORT_PATH_KEY = "sonar.items.coverage.reportPath";
  public static final String IT_REPORT_PATH_KEY = "sonar.items.coverage.itReportPath";
  public static final String OVERALL_REPORT_PATH_KEY = "sonar.items.coverage.overallReportPath";
  ...
  public static final String FORCE_ZERO_COVERAGE_KEY = "sonar.items.coverage.forceZeroCoverage";
   ...
   * {@inheritDoc}
   */
  public ItemsCoverageSensor(Settings settings, FileSystem fs, ProjectReactor reactor) {
  ...
  public void analyse(Project project, SensorContext context) {
    ...
    ItemsUtils.LOG.debug("Parsing coverage reports");
    ...
    ItemsUtils.LOG.debug("Parsing integration test coverage reports");
    ...
    ItemsUtils.LOG.debug("Parsing overall test coverage reports");
    ...
    if (isForceZeroCoverageActivated()) {
      ItemsUtils.LOG.debug("Zeroing coverage information for untouched files");
      ...
      for (CoverageParser parser : parsers) {
        try {
          ...
          if (!measuresForReport.isEmpty()) {
            ...
            ItemsUtils.LOG.info("Added report '{}' (parsed by: {}) to the coverage data", report, parser);
            break;
          }
        } catch (XMLStreamException e) {
          ItemsUtils.LOG.trace("Report {} cannot be parsed by {}", report, parser);
        }
      }

      if (!parsed) {
        ItemsUtils.LOG.error("Report {} cannot be parsed", report);
      }
    }
  ...
  private void saveMeasures(Project project,
    ...
    for (Map.Entry<String, CoverageMeasuresBuilder> entry : coverageMeasures.entrySet()) {
      ...
      if (fileExist(context, cxxfile)) {
        ItemsUtils.LOG.debug("Saving coverage measures for file '{}'", filePath);
        ...
        ItemsUtils.LOG.warn("Cannot find the file '{}', ignoring coverage measures", filePath);
      }
    }
  }
  ...
  private void zeroMeasuresWithoutReports(Project project,
    ...
    for (File file : fs.files(fs.predicates().hasLanguage(ItemsLanguage.KEY))) {
      org.sonar.api.resources.File resource = org.sonar.api.resources.File.fromIOFile(file, project);
      if (fileExist(context, resource)) {
        String filePath = ItemsUtils.normalizePath(file.getAbsolutePath());
  ...
  private void saveZeroValueForResource(org.sonar.api.resources.File resource,
    ...
    if (ncloc != null && stmts != null
      ...
      ItemsUtils.LOG.debug("Zeroing {}coverage measures for file '{}'", coverageKind, filePath);
