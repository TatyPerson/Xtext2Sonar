package org.sonar.Items.plugin.xunit;
...
import org.sonar.Items.api.ItemsAstScanner;
import org.sonar.Items.ItemsConfiguration;
import org.sonar.Items.plugin.ItemsLanguage;
import org.sonar.Items.plugin.ItemsPlugin;
import org.sonar.Items.plugin.utils.ItemsReportSensor;
import org.sonar.Items.plugin.utils.ItemsUtils;
import org.sonar.Items.plugin.utils.EmptyReportException;
 ...
 * {@inheritDoc}
 */
public class ItemsXunitSensor extends ItemsReportSensor {
  public static final String REPORT_PATH_KEY = "sonar.items.xunit.reportPath";
  public static final String XSLT_URL_KEY = "sonar.items.xunit.xsltURL";
  public static final String PROVIDE_DETAILS_KEY = "sonar.items.xunit.provideDetails";
   ...
   * {@inheritDoc}
   */
  public ItemsXunitSensor(Settings conf, FileSystem fs, ProjectReactor reactor) {
  ...
  public boolean shouldExecuteOnProject(Project project) {
    ...
    return !project.getFileSystem().mainFiles(ItemsLanguage.KEY).isEmpty();
  }
    ...
    try{
      ...
      if (!reports.isEmpty()) {
        ...
        for (File report : reports) {
          ItemsUtils.LOG.info("Processing report '{}'", report);
          try {
            parser.parse(transformReport(report));
          } catch (EmptyReportException e) {
            ItemsUtils.LOG.warn("The report '{}' seems to be empty, ignoring.", report);
          }
        }
        List<TestCase> testcases = parserHandler.getTestCases();

        ItemsUtils.LOG.info("Parsing 'xUnit' format");
        boolean providedetails = conf.getBoolean(PROVIDE_DETAILS_KEY);
        if (providedetails) {
        ...
        ItemsUtils.LOG.debug("No reports found, nothing to process");
      }
    } catch (Exception e) {
  ...
  {
    ItemsUtils.LOG.info("Processing in 'simple mode' i.e. with provideDetails=false.");
    ...
    if (testsCount > 0) {
      ...
      ItemsUtils.LOG.debug("The reports contain no testcases");
    }
  }
  ...
  private void detailledMode(final Project project, final SensorContext context, List<TestCase> testcases)
    ...
    javax.xml.transform.TransformerException
  {
    ItemsUtils.LOG.info("Processing in 'detailled mode' i.e. with provideDetails=true");

    String sonarTests = conf.getString("sonar.tests");
    if (sonarTests == null || "".equals(sonarTests)){
      ItemsUtils.LOG.error("The property 'sonar.tests' is unset. Please set it to proceed");
      return;
    }
    ...
    ItemsUtils.LOG.info("Summary: testcases processed = {}, skipped = {}", tcTotal, tcSkipped);
    if (tcSkipped > 0){
      ItemsUtils.LOG.warn("Some testcases had to be skipped, check the relevant parts of your setup "
  ...
  private Collection<TestResource> lookupResources(Project project, SensorContext context, List<TestCase> testcases) {
    ...
    for (TestCase tc : testcases) {
      ...
      ItemsUtils.LOG.debug("Trying the resource for the testcase '{}' ...", tc.getFullname());
      org.sonar.api.resources.File sonarResource = lookupResource(project, context, tc);
      if (sonarResource != null) {
        ItemsUtils.LOG.debug("... found! The resource is '{}'", sonarResource);
        ...
        tcSkipped++;
        ItemsUtils.LOG.warn("... no resource found, the testcase '{}' has to be skipped",
                          tc.getFullname());
      }
    }
      ...
      throws java.io.IOException, javax.xml.transform.TransformerException
  {
    ...
    if (xsltURL != null && report.length() > 0) {
      ItemsUtils.LOG.debug("Transforming the report using xslt '{}'", xsltURL);
      InputStream inputStream = this.getClass().getResourceAsStream("/xsl/" + xsltURL);
      if (inputStream == null) {
        ItemsUtils.LOG.debug("Transforming: try to access external XSLT via URL");
      ...
      ItemsUtils.LOG.debug("Transformation skipped: no xslt given");
    }
  ...
  private org.sonar.api.resources.File lookupResource(Project project, SensorContext context, TestCase tc) {
    ...
    if (filepath != null){
      ItemsUtils.LOG.debug("Performing the 'filename'-based lookup using the value '{}'", filepath);
    ...
    if (classname != null){
      ...
      ItemsUtils.LOG.debug("Performing AST-based lookup, determined file path: '{}'", filepath);
      sonarResource = lookupInSonar(filepath, context, project);
    } else {
      ItemsUtils.LOG.debug("Skipping the AST-based lookup: no classname provided");
    }
  ...
  void buildLookupTables() {
    ...
    Iterable<File> files = fs.files(predicates.and(
        ...
        predicates.hasLanguage(ItemsLanguage.KEY)));

    ItemsConfiguration cxxConf = new ItemsConfiguration(fs.encoding());
    cxxConf.setBaseDir(fs.baseDir().getAbsolutePath());
    for (File file : files) {
      @SuppressWarnings("unchecked")
      SourceFile source = ItemsAstScanner.scanSingleFileConfig(file, cxxConf);
