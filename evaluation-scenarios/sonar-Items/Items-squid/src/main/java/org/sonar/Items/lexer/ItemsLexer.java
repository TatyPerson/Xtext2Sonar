...
import org.sonar.Items.channels.CharacterLiteralsChannel;
import org.sonar.Items.channels.StringLiteralsChannel;
import org.sonar.Items.api.ItemsKeyword;
import org.sonar.Items.api.ItemsPunctuator;
...
public final class ItemsLexer {

  private ItemsLexer() {
  }

  public static Lexer create() {
    return create(new ItemsConfiguration());
  }
  ...
  public static Lexer create(ItemsConfiguration conf) {
    Lexer.Builder builder = Lexer.builder()
        ...
        .withChannel(regexp(ItemsTokenType.NUMBER, "[0-9]++\\.[0-9]*+" + opt(EXP) + opt(FLOAT_SUFFIX)))
        .withChannel(regexp(ItemsTokenType.NUMBER, "\\.[0-9]++" + opt(EXP) + opt(FLOAT_SUFFIX)))
        .withChannel(regexp(ItemsTokenType.NUMBER, "[0-9]++" + EXP + opt(FLOAT_SUFFIX)))

        // C++ Standard, Section 2.14.2 "Integer literals"
        .withChannel(regexp(ItemsTokenType.NUMBER, "[1-9][0-9]*+" + opt(INTEGER_SUFFIX))) // Decimal literals
        .withChannel(regexp(ItemsTokenType.NUMBER, "0[0-7]++" + opt(INTEGER_SUFFIX))) // Octal Literals
        .withChannel(regexp(ItemsTokenType.NUMBER, "0[xX][0-9a-fA-F]++" + opt(INTEGER_SUFFIX))) // Hex Literals
        .withChannel(regexp(ItemsTokenType.NUMBER, "0" + opt(INTEGER_SUFFIX))) // Decimal zero
        ...
        .withChannel(new IdentifierAndKeywordChannel(and("[a-zA-Z_]", o2n("\\w")), true, ItemsKeyword.values()))

        // C++ Standard, Section 2.13 "Operators and punctuators"
        .withChannel(new PunctuatorChannel(ItemsPunctuator.values()))
