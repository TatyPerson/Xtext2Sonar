package org.sonar.Persistence.plugin.xunit;
...
import org.sonar.Persistence.api.PersistenceAstScanner;
import org.sonar.Persistence.PersistenceConfiguration;
import org.sonar.Persistence.plugin.PersistenceLanguage;
import org.sonar.Persistence.plugin.PersistencePlugin;
import org.sonar.Persistence.plugin.utils.PersistenceReportSensor;
import org.sonar.Persistence.plugin.utils.PersistenceUtils;
import org.sonar.Persistence.plugin.utils.EmptyReportException;
 ...
 * {@inheritDoc}
 */
public class PersistenceXunitSensor extends PersistenceReportSensor {
  public static final String REPORT_PATH_KEY = "sonar.persistence.xunit.reportPath";
  public static final String XSLT_URL_KEY = "sonar.persistence.xunit.xsltURL";
  public static final String PROVIDE_DETAILS_KEY = "sonar.persistence.xunit.provideDetails";
   ...
   * {@inheritDoc}
   */
  public PersistenceXunitSensor(Settings conf, FileSystem fs, ProjectReactor reactor) {
  ...
  public boolean shouldExecuteOnProject(Project project) {
    ...
    return !project.getFileSystem().mainFiles(PersistenceLanguage.KEY).isEmpty();
  }
    ...
    try{
      ...
      if (!reports.isEmpty()) {
        ...
        for (File report : reports) {
          PersistenceUtils.LOG.info("Processing report '{}'", report);
          try {
            parser.parse(transformReport(report));
          } catch (EmptyReportException e) {
            PersistenceUtils.LOG.warn("The report '{}' seems to be empty, ignoring.", report);
          }
        }
        List<TestCase> testcases = parserHandler.getTestCases();

        PersistenceUtils.LOG.info("Parsing 'xUnit' format");
        boolean providedetails = conf.getBoolean(PROVIDE_DETAILS_KEY);
        if (providedetails) {
        ...
        PersistenceUtils.LOG.debug("No reports found, nothing to process");
      }
    } catch (Exception e) {
      String msg = new StringBuilder()
        ...
        .append("'")
        .toString();
      PersistenceUtils.LOG.error(msg);
  ...
  {
    PersistenceUtils.LOG.info("Processing in 'simple mode' i.e. with provideDetails=false.");
    ...
    if (testsCount > 0) {
      ...
      PersistenceUtils.LOG.debug("The reports contain no testcases");
    }
  }
  ...
  private void detailledMode(final Project project, final SensorContext context, List<TestCase> testcases)
    ...
    javax.xml.transform.TransformerException
  {
    PersistenceUtils.LOG.info("Processing in 'detailled mode' i.e. with provideDetails=true");

    String sonarTests = conf.getString("sonar.tests");
    if (sonarTests == null || "".equals(sonarTests)){
      PersistenceUtils.LOG.error("The property 'sonar.tests' is unset. Please set it to proceed");
      return;
    }
    ...
    PersistenceUtils.LOG.info("Summary: testcases processed = {}, skipped = {}", tcTotal, tcSkipped);
    if (tcSkipped > 0){
      PersistenceUtils.LOG.warn("Some testcases had to be skipped, check the relevant parts of your setup "
  ...
  private Collection<TestResource> lookupResources(Project project, SensorContext context, List<TestCase> testcases) {
    ...
    for (TestCase tc : testcases) {
      ...
      PersistenceUtils.LOG.debug("Trying the resource for the testcase '{}' ...", tc.getFullname());
      org.sonar.api.resources.File sonarResource = lookupResource(project, context, tc);
      if (sonarResource != null) {
        PersistenceUtils.LOG.debug("... found! The resource is '{}'", sonarResource);
        ...
        tcSkipped++;
        PersistenceUtils.LOG.warn("... no resource found, the testcase '{}' has to be skipped",
                          tc.getFullname());
      }
    }
      ...
      throws java.io.IOException, javax.xml.transform.TransformerException
  {
    ...
    if (xsltURL != null && report.length() > 0) {
      PersistenceUtils.LOG.debug("Transforming the report using xslt '{}'", xsltURL);
      InputStream inputStream = this.getClass().getResourceAsStream("/xsl/" + xsltURL);
      if (inputStream == null) {
        PersistenceUtils.LOG.debug("Transforming: try to access external XSLT via URL");
      ...
      PersistenceUtils.LOG.debug("Transformation skipped: no xslt given");
    }
  ...
  private org.sonar.api.resources.File lookupResource(Project project, SensorContext context, TestCase tc) {
    ...
    if (filepath != null){
      PersistenceUtils.LOG.debug("Performing the 'filename'-based lookup using the value '{}'", filepath);
    ...
    if (classname != null){
      ...
      PersistenceUtils.LOG.debug("Performing AST-based lookup, determined file path: '{}'", filepath);
      sonarResource = lookupInSonar(filepath, context, project);
    } else {
      PersistenceUtils.LOG.debug("Skipping the AST-based lookup: no classname provided");
    }
  ...
  void buildLookupTables() {
    ...
    Iterable<File> files = fs.files(predicates.and(
        ...
        predicates.hasLanguage(PersistenceLanguage.KEY)));

    PersistenceConfiguration cxxConf = new PersistenceConfiguration(fs.encoding());
    cxxConf.setBaseDir(fs.baseDir().getAbsolutePath());
    for (File file : files) {
      @SuppressWarnings("unchecked")
      SourceFile source = PersistenceAstScanner.scanSingleFileConfig(file, cxxConf);
