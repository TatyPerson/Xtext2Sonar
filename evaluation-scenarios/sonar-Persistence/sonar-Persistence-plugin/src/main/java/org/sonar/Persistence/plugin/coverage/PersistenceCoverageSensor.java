 ...
 * {@inheritDoc}
 */
public class PersistenceCoverageSensor extends PersistenceReportSensor {
  private enum CoverageType {
    UT_COVERAGE, IT_COVERAGE, OVERALL_COVERAGE
  }

  public static final String REPORT_PATH_KEY = "sonar.persistence.coverage.reportPath";
  public static final String IT_REPORT_PATH_KEY = "sonar.persistence.coverage.itReportPath";
  public static final String OVERALL_REPORT_PATH_KEY = "sonar.persistence.coverage.overallReportPath";
  ...
  public static final String FORCE_ZERO_COVERAGE_KEY = "sonar.persistence.coverage.forceZeroCoverage";
   ...
   * {@inheritDoc}
   */
  public PersistenceCoverageSensor(Settings settings, FileSystem fs, ProjectReactor reactor) {
  ...
  public void analyse(Project project, SensorContext context) {
    ...
    PersistenceUtils.LOG.debug("Parsing coverage reports");
    ...
    PersistenceUtils.LOG.debug("Parsing integration test coverage reports");
    ...
    PersistenceUtils.LOG.debug("Parsing overall test coverage reports");
    ...
    if (isForceZeroCoverageActivated()) {
      PersistenceUtils.LOG.debug("Zeroing coverage information for untouched files");
      ...
      for (CoverageParser parser : parsers) {
        try {
          ...
          if (!measuresForReport.isEmpty()) {
            ...
            PersistenceUtils.LOG.info("Added report '{}' (parsed by: {}) to the coverage data", report, parser);
            break;
          }
        } catch (XMLStreamException e) {
          PersistenceUtils.LOG.trace("Report {} cannot be parsed by {}", report, parser);
        }
      }

      if (!parsed) {
        PersistenceUtils.LOG.error("Report {} cannot be parsed", report);
      }
    }
  ...
  private void saveMeasures(Project project,
    ...
    for (Map.Entry<String, CoverageMeasuresBuilder> entry : coverageMeasures.entrySet()) {
      ...
      if (fileExist(context, cxxfile)) {
        PersistenceUtils.LOG.debug("Saving coverage measures for file '{}'", filePath);
        ...
        PersistenceUtils.LOG.warn("Cannot find the file '{}', ignoring coverage measures", filePath);
      }
    }
  }
  ...
  private void zeroMeasuresWithoutReports(Project project,
    ...
    for (File file : fs.files(fs.predicates().hasLanguage(PersistenceLanguage.KEY))) {
      org.sonar.api.resources.File resource = org.sonar.api.resources.File.fromIOFile(file, project);
      if (fileExist(context, resource)) {
        String filePath = PersistenceUtils.normalizePath(file.getAbsolutePath());
  ...
  private void saveZeroValueForResource(org.sonar.api.resources.File resource,
    ...
    if (ncloc != null && stmts != null
      ...
      PersistenceUtils.LOG.debug("Zeroing {}coverage measures for file '{}'", coverageKind, filePath);
