package org.sonar.Persistence.plugin.highlighter;
...
public class PersistenceHighlighter {

  private Lexer lexer;
  private Charset charset;

  public PersistenceHighlighter(PersistenceConfiguration conf) {
    this.lexer = PersistenceLexer.create(conf);
  ...
  private void highlightNonComments(Highlightable.HighlightingBuilder highlighting, List<Token> tokens, SourceFileOffsets offsets) {
    for (Token token : tokens) {
      if (PersistenceTokenType.STRING.equals(token.getType())) {
  ...
  private boolean isConstant(TokenType type) {
    return PersistenceTokenType.NUMBER.equals(type)
      || PersistenceTokenType.CHARACTER.equals(type);
  }

  private boolean isKeyword(TokenType type) {
    for (TokenType keywordType : PersistenceKeyword.values()) {
