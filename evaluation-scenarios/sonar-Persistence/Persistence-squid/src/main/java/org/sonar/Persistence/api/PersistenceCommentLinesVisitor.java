...
public class PersistenceCommentLinesVisitor <GRAMMAR extends Grammar> extends SquidAstVisitor<GRAMMAR> implements AstAndTokenVisitor {

  private final MetricDef metric;
  private Set<Integer> comments = Sets.newHashSet();
  private boolean seenFirstToken;
  
  public PersistenceCommentLinesVisitor(MetricDef metric) {
	    this.metric = metric;
  }
  ...
  public void leaveNode(AstNode astNode) {
    ...
    sourceCode.setMeasure(PersistenceMetric.COMMENT_LINES, commentlines);
  }

  public void leaveFile(AstNode ast) {
	getContext().peekSourceCode().add(metric, comments.size());
    getContext().peekSourceCode().setMeasure(PersistenceMetric.COMMENT_LINES, comments.size());
    comments.clear();
  }
}
