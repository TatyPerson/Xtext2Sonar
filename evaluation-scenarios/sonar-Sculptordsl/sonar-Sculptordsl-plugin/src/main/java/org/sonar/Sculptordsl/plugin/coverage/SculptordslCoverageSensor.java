 ...
 * {@inheritDoc}
 */
public class SculptordslCoverageSensor extends SculptordslReportSensor {
  private enum CoverageType {
    UT_COVERAGE, IT_COVERAGE, OVERALL_COVERAGE
  }

  public static final String REPORT_PATH_KEY = "sonar.sculptordsl.coverage.reportPath";
  public static final String IT_REPORT_PATH_KEY = "sonar.sculptordsl.coverage.itReportPath";
  public static final String OVERALL_REPORT_PATH_KEY = "sonar.sculptordsl.coverage.overallReportPath";
  ...
  public static final String FORCE_ZERO_COVERAGE_KEY = "sonar.sculptordsl.coverage.forceZeroCoverage";
   ...
   * {@inheritDoc}
   */
  public SculptordslCoverageSensor(Settings settings, FileSystem fs, ProjectReactor reactor) {
  ...
  public void analyse(Project project, SensorContext context) {
    ...
    SculptordslUtils.LOG.debug("Parsing coverage reports");
    ...
    SculptordslUtils.LOG.debug("Parsing integration test coverage reports");
    ...
    SculptordslUtils.LOG.debug("Parsing overall test coverage reports");
    ...
    if (isForceZeroCoverageActivated()) {
      SculptordslUtils.LOG.debug("Zeroing coverage information for untouched files");
      ...
      for (CoverageParser parser : parsers) {
        try {
          ...
          if (!measuresForReport.isEmpty()) {
            ...
            SculptordslUtils.LOG.info("Added report '{}' (parsed by: {}) to the coverage data", report, parser);
            break;
          }
        } catch (XMLStreamException e) {
          SculptordslUtils.LOG.trace("Report {} cannot be parsed by {}", report, parser);
        }
      }

      if (!parsed) {
        SculptordslUtils.LOG.error("Report {} cannot be parsed", report);
      }
    }
  ...
  private void saveMeasures(Project project,
    ...
    for (Map.Entry<String, CoverageMeasuresBuilder> entry : coverageMeasures.entrySet()) {
      ...
      if (fileExist(context, cxxfile)) {
        SculptordslUtils.LOG.debug("Saving coverage measures for file '{}'", filePath);
        ...
        SculptordslUtils.LOG.warn("Cannot find the file '{}', ignoring coverage measures", filePath);
      }
    }
  }
  ...
  private void zeroMeasuresWithoutReports(Project project,
    ...
    for (File file : fs.files(fs.predicates().hasLanguage(SculptordslLanguage.KEY))) {
      org.sonar.api.resources.File resource = org.sonar.api.resources.File.fromIOFile(file, project);
      if (fileExist(context, resource)) {
        String filePath = SculptordslUtils.normalizePath(file.getAbsolutePath());
  ...
  private void saveZeroValueForResource(org.sonar.api.resources.File resource,
    ...
    if (ncloc != null && stmts != null
      ...
      SculptordslUtils.LOG.debug("Zeroing {}coverage measures for file '{}'", coverageKind, filePath);
