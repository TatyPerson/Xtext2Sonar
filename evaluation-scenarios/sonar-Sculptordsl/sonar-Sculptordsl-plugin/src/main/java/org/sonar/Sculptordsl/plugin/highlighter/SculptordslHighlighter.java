package org.sonar.Sculptordsl.plugin.highlighter;
...
public class SculptordslHighlighter {

  private Lexer lexer;
  private Charset charset;

  public SculptordslHighlighter(SculptordslConfiguration conf) {
    this.lexer = SculptordslLexer.create(conf);
  ...
  private void highlightNonComments(Highlightable.HighlightingBuilder highlighting, List<Token> tokens, SourceFileOffsets offsets) {
    for (Token token : tokens) {
      if (SculptordslTokenType.STRING.equals(token.getType())) {
  ...
  private boolean isConstant(TokenType type) {
    return SculptordslTokenType.NUMBER.equals(type)
      || SculptordslTokenType.CHARACTER.equals(type);
  }

  private boolean isKeyword(TokenType type) {
    for (TokenType keywordType : SculptordslKeyword.values()) {
