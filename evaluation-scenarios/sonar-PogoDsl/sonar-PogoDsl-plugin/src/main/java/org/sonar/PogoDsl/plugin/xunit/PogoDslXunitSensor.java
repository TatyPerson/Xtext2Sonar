package org.sonar.PogoDsl.plugin.xunit;
...
import org.sonar.PogoDsl.plugin.utils.EmptyReportException;
 ...
 * {@inheritDoc}
 */
public class PogoDslXunitSensor extends PogoDslReportSensor {
  public static final String REPORT_PATH_KEY = "sonar.pogodsl.xunit.reportPath";
  public static final String XSLT_URL_KEY = "sonar.pogodsl.xunit.xsltURL";
  public static final String PROVIDE_DETAILS_KEY = "sonar.pogodsl.xunit.provideDetails";
   ...
   * {@inheritDoc}
   */
  public PogoDslXunitSensor(Settings conf, FileSystem fs, ProjectReactor reactor) {
  ...
  public boolean shouldExecuteOnProject(Project project) {
    ...
    return !project.getFileSystem().mainFiles(PogoDslLanguage.KEY).isEmpty();
  }
    ...
    try{
      ...
      if (!reports.isEmpty()) {
        ...
        for (File report : reports) {
          PogoDslUtils.LOG.info("Processing report '{}'", report);
          try {
            parser.parse(transformReport(report));
          } catch (EmptyReportException e) {
            PogoDslUtils.LOG.warn("The report '{}' seems to be empty, ignoring.", report);
          }
        }
        List<TestCase> testcases = parserHandler.getTestCases();

        PogoDslUtils.LOG.info("Parsing 'xUnit' format");
        boolean providedetails = conf.getBoolean(PROVIDE_DETAILS_KEY);
        if (providedetails) {
        ...
        PogoDslUtils.LOG.debug("No reports found, nothing to process");
      }
    } catch (Exception e) {
  ...
  {
    PogoDslUtils.LOG.info("Processing in 'simple mode' i.e. with provideDetails=false.");
    ...
    if (testsCount > 0) {
      ...
      PogoDslUtils.LOG.debug("The reports contain no testcases");
    }
  }
  ...
  private void detailledMode(final Project project, final SensorContext context, List<TestCase> testcases)
    ...
    javax.xml.transform.TransformerException
  {
    PogoDslUtils.LOG.info("Processing in 'detailled mode' i.e. with provideDetails=true");

    String sonarTests = conf.getString("sonar.tests");
    if (sonarTests == null || "".equals(sonarTests)){
      PogoDslUtils.LOG.error("The property 'sonar.tests' is unset. Please set it to proceed");
      return;
    }
    ...
    PogoDslUtils.LOG.info("Summary: testcases processed = {}, skipped = {}", tcTotal, tcSkipped);
    if (tcSkipped > 0){
      PogoDslUtils.LOG.warn("Some testcases had to be skipped, check the relevant parts of your setup "
  ...
  private Collection<TestResource> lookupResources(Project project, SensorContext context, List<TestCase> testcases) {
    ...
    for (TestCase tc : testcases) {
      ...
      PogoDslUtils.LOG.debug("Trying the resource for the testcase '{}' ...", tc.getFullname());
      org.sonar.api.resources.File sonarResource = lookupResource(project, context, tc);
      if (sonarResource != null) {
        PogoDslUtils.LOG.debug("... found! The resource is '{}'", sonarResource);
        ...
        tcSkipped++;
        PogoDslUtils.LOG.warn("... no resource found, the testcase '{}' has to be skipped",
                          tc.getFullname());
      }
    }
      ...
      throws java.io.IOException, javax.xml.transform.TransformerException
  {
    ...
    if (xsltURL != null && report.length() > 0) {
      PogoDslUtils.LOG.debug("Transforming the report using xslt '{}'", xsltURL);
      InputStream inputStream = this.getClass().getResourceAsStream("/xsl/" + xsltURL);
      if (inputStream == null) {
        PogoDslUtils.LOG.debug("Transforming: try to access external XSLT via URL");
      ...
      PogoDslUtils.LOG.debug("Transformation skipped: no xslt given");
    }
  ...
  private org.sonar.api.resources.File lookupResource(Project project, SensorContext context, TestCase tc) {
    ...
    if (filepath != null){
      PogoDslUtils.LOG.debug("Performing the 'filename'-based lookup using the value '{}'", filepath);
    ...
    if (classname != null){
      ...
      PogoDslUtils.LOG.debug("Performing AST-based lookup, determined file path: '{}'", filepath);
      sonarResource = lookupInSonar(filepath, context, project);
    } else {
      PogoDslUtils.LOG.debug("Skipping the AST-based lookup: no classname provided");
    }
  ...
  void buildLookupTables() {
    ...
    PogoDslConfiguration cxxConf = new PogoDslConfiguration(fs.encoding());
    cxxConf.setBaseDir(fs.baseDir().getAbsolutePath());
    for (File file : files) {
      @SuppressWarnings("unchecked")
      SourceFile source = PogoDslAstScanner.scanSingleFileConfig(file, cxxConf);
