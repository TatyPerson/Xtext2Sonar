 ...
 * {@inheritDoc}
 */
public class PogoDslCoverageSensor extends PogoDslReportSensor {
  private enum CoverageType {
    UT_COVERAGE, IT_COVERAGE, OVERALL_COVERAGE
  }

  public static final String REPORT_PATH_KEY = "sonar.pogodsl.coverage.reportPath";
  public static final String IT_REPORT_PATH_KEY = "sonar.pogodsl.coverage.itReportPath";
  public static final String OVERALL_REPORT_PATH_KEY = "sonar.pogodsl.coverage.overallReportPath";
  ...
  public static final String FORCE_ZERO_COVERAGE_KEY = "sonar.pogodsl.coverage.forceZeroCoverage";
   ...
   * {@inheritDoc}
   */
  public PogoDslCoverageSensor(Settings settings, FileSystem fs, ProjectReactor reactor) {
  ...
  public void analyse(Project project, SensorContext context) {
    ...
    PogoDslUtils.LOG.debug("Parsing coverage reports");
    ...
    PogoDslUtils.LOG.debug("Parsing integration test coverage reports");
    ...
    PogoDslUtils.LOG.debug("Parsing overall test coverage reports");
    ...
    if (isForceZeroCoverageActivated()) {
      PogoDslUtils.LOG.debug("Zeroing coverage information for untouched files");
      ...
      for (CoverageParser parser : parsers) {
        try {
          ...
          if (!measuresForReport.isEmpty()) {
            ...
            PogoDslUtils.LOG.info("Added report '{}' (parsed by: {}) to the coverage data", report, parser);
            break;
          }
        } catch (XMLStreamException e) {
          PogoDslUtils.LOG.trace("Report {} cannot be parsed by {}", report, parser);
        }
      }

      if (!parsed) {
        PogoDslUtils.LOG.error("Report {} cannot be parsed", report);
      }
    }
  ...
  private void saveMeasures(Project project,
    ...
    for (Map.Entry<String, CoverageMeasuresBuilder> entry : coverageMeasures.entrySet()) {
      ...
      if (fileExist(context, cxxfile)) {
        PogoDslUtils.LOG.debug("Saving coverage measures for file '{}'", filePath);
        ...
        PogoDslUtils.LOG.warn("Cannot find the file '{}', ignoring coverage measures", filePath);
      }
    }
  }
  ...
  private void zeroMeasuresWithoutReports(Project project,
    ...
    for (File file : fs.files(fs.predicates().hasLanguage(PogoDslLanguage.KEY))) {
      org.sonar.api.resources.File resource = org.sonar.api.resources.File.fromIOFile(file, project);
      if (fileExist(context, resource)) {
        String filePath = PogoDslUtils.normalizePath(file.getAbsolutePath());
  ...
  private void saveZeroValueForResource(org.sonar.api.resources.File resource,
    ...
    if (ncloc != null && stmts != null
      ...
      PogoDslUtils.LOG.debug("Zeroing {}coverage measures for file '{}'", coverageKind, filePath);
