...
public class CoberturaParser implements CoverageParser {
  ...
  {
    PogoDslUtils.LOG.info("Parsing 'Cobertura' format");
      ...
      throws XMLStreamException
  {
    while (clazz.getNext() != null) {
      String normalPath = PogoDslUtils.normalizePath(clazz.getAttrValue("filename"));
      if(normalPath != null){
  ...
  private void collectFileData(SMInputCursor clazz, CoverageMeasuresBuilder builder) throws XMLStreamException {
    ...
    while (line.getNext() != null) {
      ...
      if(noHits > Integer.MAX_VALUE){
        PogoDslUtils.LOG.warn("Truncating the actual number of hits ({}) to the maximum number supported by Sonar ({})",
