package org.sonar.PogoDsl.plugin.highlighter;
...
public class PogoDslHighlighter {

  private Lexer lexer;
  private Charset charset;

  public PogoDslHighlighter(PogoDslConfiguration conf) {
    this.lexer = PogoDslLexer.create(conf);
  ...
  private void highlightNonComments(Highlightable.HighlightingBuilder highlighting, List<Token> tokens, SourceFileOffsets offsets) {
    for (Token token : tokens) {
      if (PogoDslTokenType.STRING.equals(token.getType())) {
  ...
  private boolean isConstant(TokenType type) {
    return PogoDslTokenType.NUMBER.equals(type)
      || PogoDslTokenType.CHARACTER.equals(type);
  }

  private boolean isKeyword(TokenType type) {
    for (TokenType keywordType : PogoDslKeyword.values()) {
