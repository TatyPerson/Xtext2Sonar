 ...
 * in SonarQube
 */
public abstract class NAMEReportSensor implements Sensor {
   ...
   * @param reactor
   */
  protected NAMEReportSensor(Settings conf, FileSystem fs, ProjectReactor reactor) {
   ...
   * @param reactor
   */
  protected NAMEReportSensor(ResourcePerspectives perspectives, Settings conf, FileSystem fs, ProjectReactor reactor, Metric metric) {
    this.conf = conf;
    this.fs = fs;
    this.metric = metric;
  ...
  public boolean shouldExecuteOnProject(Project project) {
    return fs.hasFiles(fs.predicates().hasLanguage(NAMELanguage.KEY));
  }
  ...
  public void analyse(Project project, SensorContext context) {
    if (!NAMEUtils.isReactorProject(project)) {
      try {
      List<File> reports = getReports(conf, reactor.getRoot().getBaseDir().getCanonicalPath(),
        ...
        try {
          ...
          NAMEUtils.LOG.info("{} processed = {}", metric == null ? "Issues" : metric.getName(),
